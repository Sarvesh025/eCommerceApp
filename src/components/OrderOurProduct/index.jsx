...
export default function index({ img, desc, id, setShow }) {
    ...
    function handelChange(e){
        if(e.target.name === 'email'){
            ...
            setForm({...form, [e.target.name] : e.target.value});
            if(postal_code_verification(e.target.value)){
                setVerify({...verify, pincode : true});
            }
            else{
                setVerify({...verify, pincode : false});
            }
            // postal_code_verification(e.target.value).then(
            //     ()=>{
            //         setVerify({...verify, pincode : true});
            //     }
            // ).catch(()=>{ setVerify({...verify, pincode : false});})
        }
    ...
    const handelOrder = (e) => {
        ...
        if(verify.email && verify.pincode && verify.small && verify.medium && verify.large && verify.extraLarge){
        if(form.small || form.medium || form.large || form.extraLarge){
            alert(`
            CUSTOMER'S DETAILS :-
            Email : ${form.email}
